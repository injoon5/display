import { resolvePathType } from "./resolve.js";
...
export function exprPathKey(expr) {
    return expr.kind === "path" ? expr.segments.join(".") : null;
}
export function extractTruthyGuards(expr) {
...
export function inferExpressionType(expr, context, guards = new Set()) {
    switch (expr.kind) {
        case "literal":
...
export function typecheckCard(card, resolve) {
    const context = {
