...
function formatTime(value: number, withSeconds: boolean): string {
  const date = new Date(value * 1000);
...
function colorScale(value: number, a: string, b: string, c?: string): string {
  const clamped = Math.max(0, Math.min(1, value));
  const left = parseColor(a);
  const middle = parseColor(b);
  if (!c) {
    return normaliseColorLiteral(
      `#${interpolateChannel(left.r, middle.r, clamped).toString(16).padStart(2, "0")}${interpolateChannel(left.g, middle.g, clamped).toString(16).padStart(2, "0")}${interpolateChannel(left.b, middle.b, clamped).toString(16).padStart(2, "0")}`
    );
  }
  ...
  return normaliseColorLiteral(
    `#${interpolateChannel(start.r, end.r, segment).toString(16).padStart(2, "0")}${interpolateChannel(start.g, end.g, segment).toString(16).padStart(2, "0")}${interpolateChannel(start.b, end.b, segment).toString(16).padStart(2, "0")}`
  );
}

export function applyFilter(name: string, input: unknown, args: unknown[]): unknown {
  switch (name) {
    case "round":
