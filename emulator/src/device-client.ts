...
export class DeviceClient {
  ...
  > {
    ...
    const res = await fetch(new URL("/device/sync", this.siteUrl), { headers });
    if (res.status === 304) {
  ...
  > {
    ...
    const res = await fetch(new URL("/device/data", this.siteUrl), { headers });
    if (res.status === 304) {
      return { kind: "notModified", etag: stripEtag(res.headers.get("etag")) };
    }
    if (!res.ok) {
  ...
  async heartbeat(body: Record<string, unknown>): Promise<void> {
    const res = await fetch(new URL("/device/heartbeat", this.siteUrl), {
      method: "POST",
